...
def index():
   # get annual sales rank
	...
	# 獲取表頭
    sql = "SHOW FIELDS FROM student.student_table"
    cursor.execute(sql)
                    ...
                    # 6.2.2 人脸数增加 / Face cnt increase: 0->1, 0->2, ..., 1->2, ...
                        ...
                        for k in range(len(faces)):
                            ...
                            if min(self.current_frame_face_X_e_distance_list) < 0.4:
                                ...
                                sql='update student_table set attendance=1 where student_name=%s'
                                try:
                                    cursor.execute(sql,(self.face_name_known_list[similar_person_num]))
                                    conn.commit()
                                    print("成功")
                                ...
                                #print(self.face_name_known_list[similar_person_num])
                            else:
...
@apppp.route("/accounts") #代表我們要處理的網站路徑
def accounts():
    return render_template("accounts.html")

@apppp.route("/add-product")
def addproduct():
